...
l <- ggplot(Conjunto_Prome,aes(x=Fecha,y=TempCelcius,group = 1))+geom_line(color ="red")+xlab("Meses")+ylab("TempCelsius(°C)")

grid.arrange(g,h,i,j,k,l,nrow = 6, ncol = 1)

## Gráfico de Relación de variables 

m <- ggplot(datos_climaticos,aes(x=Fecha,y=HumedadR,group = 1))+geom_point()
n <- ggplot(datos_climaticos,aes(x=Fecha,y=V.Viento,group = 1))+geom_point()
o <- ggplot(datos_climaticos,aes(x=Fecha,y=Lluvia,group = 1))+geom_point()
p <- ggplot(datos_climaticos,aes(x=Fecha,y=Irradiacion,group = 1))+geom_point()
q <- ggplot(datos_climaticos,aes(x=Fecha,y=EvapTransp,group = 1))+geom_point()
r <- ggplot(datos_climaticos,aes(x=Fecha,y=TempCelcius,group = 1))+geom_point()

grid.arrange(m,n,o,p,q,r,nrow = 6, ncol = 1)
